...
> **Hackathon deadline:** Aug 31, 3:00 PM PT (22 days out)
> **Devpost:** https://3d-websites-hackathon.devpost.com/
> **Prize pool (as listed):** $55
> **Final submit step:** https://devpost.com/submit-to/30501-3d-websites-hackathon/manage/submissions/1131636/finalization
...
### 0A.1 This repository's exclusive port block: `4100`–`4109`

| Port   | Service                                                                  |
| ------ | ------------------------------------------------------------------------ |
| `4100` | Vite dev server (immersive gallery)                                      |
| `4101` | Vite preview server (production build, used for load-budget measurement) |
| `4102` | Playwright webServer for fixed-camera projection regression              |
| `4103` | Static evidence/report server (golden masks, IoU diffs)                  |

Unassigned ports in this block (`4104`, `4105`, `4106`, `4107`, `4108`, `4109`) are reserved for services discovered later. **Never allocate a port outside `4100`–`4109`.**
...
Ports are not the only shared namespace. Prefix every one of these with this repository's directory name, `3d-websites-hackathon`:

- `COMPOSE_PROJECT_NAME=3d-websites-hackathon` on every Docker Compose invocation, with container names prefixed to match.
- Database name and role: `3d_websites_hackathon`.
- Redis logical database index: **0** (this repository's index in the sibling set of sixteen).
- Object-storage bucket or MinIO alias: `3d-websites-hackathon`.
...
```sh
# 3d-websites-hackathon — exclusive block 4100-4109
PORT_0=4100   # Vite dev server
PORT_1=4101   # Vite preview server
PORT_2=4102   # Playwright webServer for fixed-camera projection regression
PORT_3=4103   # Static evidence/report server
```
...
An immersive web gallery of multi-view anamorphic sculptures: one static cloud of luminous fragments resolves into a completely different recognizable silhouette from each marked viewpoint, and dissolves into an intentional spatial constellation between them.

> A single 3D sculpture tells three different stories depending on where you stand, and nothing in the sculpture ever moves.
...
1. Static CDN deployment on a real domain with content-hashed immutable assets and correct long-lived cache headers.
2. Real visitor telemetry from at least three device classes via privacy-preserving RUM with no personal data collected.
3. A deploy and a rollback both executed for real, with the rollback timed and recorded.
4. An asset and font licensing register with provenance for every shipped byte.
...
**I1. Curated sculpture geometry is static across viewpoints**
...
**I2. Each authored view passes a target projection threshold at supported aspect ratios**
...
**I3. An outline cannot be required to recognize a hero silhouette**
...
**I4. Off-axis composition remains intentional and performant**
...
**I5. Generator error is reported per view and never hidden**
...
**I6. Capability loss degrades effects before silhouette-defining geometry**
...
**I7. All canvas meaning has a semantic and reduced-motion equivalent**
...
1. Implement the differentiable soft projection operator S_v over a ray-marched occupancy field and prove its gradient against finite differences on analytic test volumes before any art direction is committed.
2. Build the multi-view occupancy solver: soft occupancy grid, per-view projection loss, total-variation and sparsity regularization, annealed thresholding into discrete fragments, all inside a cancellable worker with a bounded iteration budget.
3. Compute an achievable projection-IoU upper bound for every candidate silhouette trio BEFORE committing art direction, and commit the bound as a versioned artifact so later art changes cannot silently violate it.
4. Implement sparsification from the continuous grid into instanced fragment families (rods, shards, dust) that never drops any authored view below its committed per-view IoU floor.
5. Lock every camera station: projection matrix, FOV, aspect-ratio family. Any station edit must automatically re-run solve + projection regression or fail the build.
6. Add an off-axis composition objective (arc coherence, density gradient, depth layering) with a committed numeric metric, so non-station views are authored rather than merely leftover noise.
7. Make fragment variation deterministic under an explicit seed: same seed produces a byte-identical scene payload, asserted in CI.
8. Ship the WebGPU compute path and the WebGL2/CPU fallback path and prove cross-path equivalence within a declared numeric tolerance rather than assuming parity.
9. Surface per-view residual error in the user-facing generator as an honest compatibility verdict; an incompatible trio must be reported as incompatible, never rendered as a degraded success.
10. Build the fixed-camera projection regression harness: golden masks per station per supported aspect ratio, IoU thresholds enforced in CI, visual diffs archived as evidence.
11. Implement the quality governor: capability detection, dynamic resolution, thermal and frame budget response, with the ordering rule that effects degrade before silhouette-defining geometry.
12. Build the accessibility mirror as a first-class artifact: every authored meaning has an HTML semantic equivalent and a reduced-motion crossfade path, audited automatically.
...
1. Incompatible three-view mask trio: report per-view residuals and an explicit "these cannot coexist" verdict; never quietly optimize toward whichever view is easiest.
2. Device below the capability floor: serve the semantic 2D gallery. A blank canvas or a silently missing sculpture is a defect, not a fallback.
3. Solver cancellation or timeout: propagate cancellation into the worker, restore the prior state atomically, and never render partial geometry as a finished piece.
4. WebGL context loss: recover or degrade into a designed state with a visible explanation.
5. Reduced-motion preference: crossfade between pre-rendered station states carrying identical meaning, not a stripped experience.
6. Blocked or denied audio: visible mute state and no silent failure path.
...
**1. `src/scenes` — Curated masks, solved geometry, camera stations, art direction**
...
**2. `src/solver` — Back-projection, soft occupancy, optimization, sparsification**
...
**3. `src/sculpture` — Instancing, fragment placement, materials, LOD**
...
**4. `src/camera` — Rail, snap assistance, framing, URL state**
...
**5. `src/lighting-audio` — Interactive light, projection-error sound, lifecycle**
...
**6. `src/quality` — Capability, dynamic resolution, thermal/frame budget**
...
**7. `src/accessibility` — HTML semantic mirror, 2D gallery, reduced motion, controls**
...
1. Golden projection IoU per station per aspect-ratio family, regenerated by a single committed command.
2. Off-axis composition metric sampled across a full orbit, tracked as a time series across epochs.
3. Frame-time and memory distributions across the declared device matrix, reported as p50/p95/p99 rather than a single number.
4. Time-to-first-reveal measured cold, warm, on throttled network and throttled CPU, with the budget enforced in CI.
5. Generator solve-time and residual distributions over a corpus of synthetic mask trios, including deliberately incompatible ones.
6. Automated audit proving every authored meaning has a screen-reader-reachable equivalent.
...
**G1. Three curated scenes meet projection and art review**
...
**G2. First meaningful reveal meets load budget**
...
**G3. Supported device matrix meets frame/memory targets**
...
**G4. Generator reports honest error and cannot block main render**
...
**G5. Accessibility fallback covers all authored meaning**
...
**G6. No external asset/license uncertainty remains**
...
**Submission mechanics** (deadline Aug 31, 3:00 PM PT (22 days out)):
...
| -------------------------------------------------------------- | ------- | ------- | ------- | -------------------------------------- |
| Per-station projection IoU floor                               | 0.86    | 0.90    | 0.93    | 0.95                                   |
| Time to first meaningful reveal (cold, CPU-throttled 4x)       | 2.5s    | 2.0s    | 1.6s    | 1.3s                                   |
| p95 frame time on the reference low-end device                 | 16.6ms  | 13ms    | 11ms    | 9ms                                    |
| Compressed scene payload                                       | 6MB     | 4MB     | 2.8MB   | 2.0MB                                  |
| Device matrix breadth                                          | 4       | 7       | 11      | 15                                     |
| Aspect-ratio families under projection regression              | 3       | 6       | 10      | 14                                     |
| Mutation score on solver and geometry packages                 | 55%     | 70%     | 80%     | 88%                                    |
| Automated accessibility rule count enforced at zero violations | 20      | 40      | 70      | 100                                    |
...
1. A silhouette that reads perfectly at 1440p and becomes unrecognizable at 390px width.
2. A judge on throttled conference wifi who never sees the first reveal.
3. iOS low-power mode halving the frame rate mid-orbit.
4. A GPU that silently falls back to software rendering.
5. Bloom and particle density masking a silhouette that is actually unreadable.
6. The tab being backgrounded mid-solve and resumed with stale worker state.
7. Two rapid station jumps causing camera spring overshoot past the authored view.
8. A shared URL hash loading directly into station three with no context.
9. A screen-reader user for whom the canvas does not exist at all.
10. Thermal throttling after ninety seconds of continuous orbit.
11. A 4K display at devicePixelRatio 3 blowing the fill-rate budget.
12. A long caption in a right-to-left locale overflowing world-space typography.
